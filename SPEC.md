...
| Planned (packages verified 2026-07, not yet wired) | Raspberry Pi OS 12 (bookworm) | 7.0 only | apt | full server on `armhf`+`arm64`; the 7.4 raspbian tree ships **no** ARM binaries, so RPi is 7.0-only. Installer support not yet implemented — see §18. Re-verify at build time. |
...
**Next planned platform** (§4 planned row; packages verified 2026-07, installer support not yet built): **Raspberry Pi OS 12** (`raspbian`/bookworm, family debian) as a full-server target on `armhf` and `arm64` for **7.0 only** — its 7.4 raspbian tree ships only the arch-independent `zabbix-release` config package with no ARM binaries. `detect_family` already maps `raspbian`→debian and package selection is family-based, so the remaining work is: accept `raspbian`/12 in `detect_supported`, promote its arm arches via `_arch_confirmed_for_os`, gate 7.4 off, and fix `repo.sh`'s apt URL — the raspbian bootstrap deb is named `zabbix-release_latest_<zv>+debian12_all.deb` (the `+debian12` suffix, not the `+raspbian12` that `ID`+`VERSION_ID` would yield, though the path segment stays `/raspbian/`). RPi on 7.4 must be gated off until the "reroute apt to the Debian-12 ARM repo" path is proven in a real install. A real ARM install job needs an arm64 runner or QEMU.
