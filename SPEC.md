...
| Planned (packages verified 2026-07, not yet wired) | `amzn` (2023), Raspberry Pi OS 12 (bookworm) | AL 2023: 7.0 + 7.4 · RPi 12: 7.0 only | dnf / apt | Zabbix publishes complete server package sets: Amazon Linux 2023 on `x86_64`+`aarch64`, Raspberry Pi OS 12 on `armhf`+`arm64`. The 7.4 raspbian tree ships **no** ARM binaries, so RPi is 7.0-only. Installer support not yet implemented — see §18. Re-verify at build time. |
...
**Next planned platforms** (§4 stretch row; packages verified available 2026-07, installer support not yet built): **Amazon Linux 2023** (`amzn`, family rhel) as a full-server target on `x86_64` and `aarch64` for both 7.0 and 7.4, and **Raspberry Pi OS 12** (`raspbian`/bookworm, family debian) as a full-server target on `armhf` and `arm64` for 7.0 only — its 7.4 raspbian tree ships only the arch-independent `zabbix-release` config package with no ARM binaries. `detect_family` already maps both IDs (`amzn`→rhel, `raspbian`→debian) and recommendation/package selection is family-based, so most of the pipeline needs no change. Two repo-URL specifics differ from the existing families and must be handled in `repo.sh`: the DNF release RPM lives at `.../amazonlinux/<VERSION_ID>/…` with an `.amzn<VERSION_ID>` suffix (not the rhel `.../rhel/<major>/…el<major>` shape), and the raspbian bootstrap deb is named `zabbix-release_latest_<zv>+debian12_all.deb` — the `+debian12` suffix, not the `+raspbian12` that `ID`+`VERSION_ID` would yield, even though the path segment stays `/raspbian/`. RPi on 7.4 must be gated off until the "reroute apt to the Debian-12 ARM repo" path is proven in a real install. Amazon Linux 2023 is CI-coverable directly (`amazonlinux:2023` container, x86_64); a real ARM install job for RPi needs an arm64 runner or QEMU.
