...
| macOS (agent only) | `macos` (Darwin, by `uname`) | any recent, **arm64 only** | .pkg | **Agent only** — Zabbix ships no macOS server/proxy. Installs the signed `zabbix_agentd` .pkg from cdn.zabbix.com + a launchd daemon. Needs bash ≥ 4 (macOS ships 3.2) — the one-liner uses Homebrew's; Intel Macs are unsupported (the CDN's latest listing has **no** macos-amd64 artifact in any form for 7.x — verified via CI 2026-07). |
| Windows (agent only) | n/a (PowerShell, `install.ps1`) | 10 / Server 2016+, amd64 (ARM64 via x64 emulation) | MSI | **Agent only**, via the native `install.ps1` — bash never runs on Windows. Downloads the official MSI from the same CDN `latest` pointer (Zabbix ships amd64 + i386 MSIs only; Windows-on-ARM runs the amd64 build under x64 emulation), requires a valid Zabbix Authenticode signature, installs silently (SERVER/SERVERACTIVE/HOSTNAME), health-checks service + port 10050. Classic `zabbix_agentd` by default, `-Agent2` for agent 2; `-Uninstall`/`-Yes`/`-DryRun` mirror the bash flags. |

Match on `ID` first, fall back to `ID_LIKE`. Architecture: `x86_64` fully supported; `aarch64` only where Zabbix publishes packages for that distro (*verify at build time*), otherwise print a clear unsupported message. Confirmed available for Amazon Linux 2023 (`aarch64`); macOS is agent-only on Apple Silicon (`arm64`).
...
**macOS** (`macos`, detected via `uname`) is a supported **agent-only** target on Apple Silicon (`arm64`) — Zabbix ships no macOS server/proxy, only the classic `zabbix_agentd` as a signed `.pkg` on cdn.zabbix.com. A self-contained path in `os_macos.sh` (routed from `main()` for `DETECT_OS_ID=macos`) downloads the signed `.pkg`, installs it via `installer(8)` (temp file must end in `.pkg`), points `zabbix_agentd.conf` at the server, loads the launchd daemon, and health-checks port 10050; `--uninstall` reverses it. macOS ships bash 3.2, so `core.sh` guards for bash ≥ 4 and `install.sh` hands off to a Homebrew bash. Verified end-to-end (install → running → uninstall) on a real Apple-Silicon `macos-latest` CI runner. Intel Macs are unsupported — the CDN's latest listing contains **no** macos-amd64 artifact in any form (.pkg or tar.gz) for 7.x (verified via the CI listing diagnostic, 2026-07); the macos-agent CI job keeps re-checking the listing so a future Intel build fails the job loudly as a prompt to add support.

**Windows** is a supported **agent-only** target via the native `install.ps1` (§4, §6) — a self-contained PowerShell analog of `os_macos.sh`, not part of the bash bundle. It shares the CDN `latest`-pointer idea (`.../stable/<major>/latest/zabbix_agent[2]-<major>-latest-windows-amd64-openssl.msi`; names verified against the live listing 2026-07 — amd64 and i386 only, so ARM64 machines get the amd64 MSI under x64 emulation), gates on a valid Zabbix Authenticode signature, installs with msiexec public properties (SERVER/SERVERACTIVE/HOSTNAME), health-checks the service + port 10050 (exit 6 on failure), and uninstalls by product code from the registry. Verified end-to-end (Pester units + real install/uninstall of both agentd and agent2) on a `windows-latest` CI runner.
