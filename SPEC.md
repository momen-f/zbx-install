...
**macOS** (`macos`, detected via `uname`) is a supported **agent-only** target on Apple Silicon (`arm64`) — Zabbix ships no macOS server/proxy, only the classic `zabbix_agentd` as a signed `.pkg` on cdn.zabbix.com. A self-contained path in `os_macos.sh` (routed from `main()` for `DETECT_OS_ID=macos`) downloads the signed `.pkg`, installs it via `installer(8)` (temp file must end in `.pkg`), points `zabbix_agentd.conf` at the server, loads the launchd daemon, and health-checks port 10050; `--uninstall` reverses it. macOS ships bash 3.2, so `core.sh` guards for bash ≥ 4 and `install.sh` hands off to a Homebrew bash. Verified end-to-end (install → running → uninstall) on a real Apple-Silicon `macos-latest` CI runner. Intel Macs are unsupported — Zabbix ships no macOS amd64 `.pkg`.
