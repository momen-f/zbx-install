...
10. arm64: Zabbix repos cover most but not all family/version combos → if `DETECT_ARCH=aarch64` and the repo probe (§12.1) fails, say so explicitly in the error menu ("no repo for this arch/OS combo": agent-only fallback if the agent repo resolves · exit 3). Confirmed first-class: Amazon Linux 2023 (`aarch64`) and Raspberry Pi OS 12 on 7.0 (`arm64`, `armhf`); the probe→agent-only fallback still applies to every unverified arm combo. **Debian-family ARM routing:** Zabbix ships all Debian-family ARM packages (armhf + arm64) only in the `raspbian` repo — the `debian` repo is x86-only — so `repo.sh` routes any Debian-family ARM host (including 64-bit Raspberry Pi OS, which reports `ID=debian`) to `/raspbian/` with the `+debian<ver>` suffix. Both 7.0 and 7.4 are available (7.4's bootstrap `zabbix-release` lives in `/7.4/release/raspbian/`, its ARM binaries under `/7.4/stable/raspbian/` — the same split as Amazon Linux 7.4).
...
**Raspberry Pi OS 12** (bookworm, family debian) is a supported full-server target on `armhf` and `arm64` for both **Zabbix 7.0 and 7.4**. The wrinkle is detection: 32-bit Pi OS reports `ID=raspbian`, but **64-bit Pi OS reports `ID=debian`** (indistinguishable from plain Debian by os-release), and Zabbix ships all Debian-family ARM packages only in the `raspbian` repo (the `debian` repo is x86-only). So the design keys off **arch, not hardware**: `detect_supported` accepts `raspbian`/12 (64-bit is already covered by the `debian`/12 row); `_arch_class` treats all ARM as repo-dependent `maybe`, promoted to `yes` for the verified combos via `_arch_confirmed_for_os` (amzn 2023, raspbian 12, debian 12); and `repo.sh` routes any Debian-family ARM host to `/raspbian/` with the `+debian<ver>` suffix (§15.10). The existing flat-vs-legacy probe handles both versions (7.0 flat at `/7.0/raspbian/`; 7.4's bootstrap at `/7.4/release/raspbian/`, its ARM binaries under `/7.4/stable/raspbian/` — the same split as Amazon Linux 7.4). Verified by unit tests (detect + URL routing) and live repo probes cross-checked against the official download selector; **no real ARM install has been run** — this project's CI is x86-only and there is no arm runner/QEMU path, so unlike Amazon Linux (which reuses the real-install-tested rhel x86 path), the Debian-family install pipeline has never executed on ARM. This is the one shipped platform without an end-to-end install behind it.
