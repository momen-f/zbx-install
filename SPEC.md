...
**Non-goals (v1):** HA clusters, upgrading existing Zabbix installs between major versions, TLS certificate automation (print guidance only), Docker/K8s deployment targets, remote multi-host orchestration. (Windows agents were originally a non-goal; since 0.4.0 they are supported via the native `install.ps1`, §4/§6.) A **PostgreSQL-backed proxy** is out of scope (proxy supports SQLite3 or MySQL/MariaDB only), and the installer never auto-registers a proxy on the server — no upstream API exists for that; it prints the manual registration step instead (§15.9).
...
| macOS (agent only) | `macos` (Darwin, by `uname`) | any recent, arm64 + x86_64 | .pkg / tar.gz | **Agent only** — Zabbix ships no macOS server/proxy. Apple Silicon installs the signed `zabbix_agentd` .pkg from cdn.zabbix.com; Intel installs the tar.gz archive into the same layout with a hand-written launchd plist (same label, so config/service/health/uninstall are shared). Needs bash ≥ 4 (macOS ships 3.2) — the one-liner uses Homebrew's. |
| Windows (agent only) | n/a (PowerShell, `install.ps1`) | 10 / Server 2016+, amd64 (+arm64 where Zabbix ships an MSI) | MSI | **Agent only**, via the native `install.ps1` — bash never runs on Windows. Downloads the official MSI from the same CDN `latest` pointer, requires a valid Zabbix Authenticode signature, installs silently (SERVER/SERVERACTIVE/HOSTNAME), health-checks service + port 10050. Classic `zabbix_agentd` by default, `-Agent2` for agent 2; `-Uninstall`/`-Yes`/`-DryRun` mirror the bash flags. |

Match on `ID` first, fall back to `ID_LIKE`. Architecture: `x86_64` fully supported; `aarch64` only where Zabbix publishes packages for that distro (*verify at build time*), otherwise print a clear unsupported message. Confirmed available for Amazon Linux 2023 (`aarch64`); macOS is agent-only on Apple Silicon (`arm64`) and Intel (`x86_64`, tar.gz).
...
3. Channel selection: default = latest GitHub Release asset (`dist/zbx-install.sh` + `SHA256SUMS`, checksum verified with `sha256sum -c`, falling back to `shasum -a 256 -c` on stock macOS which has no sha256sum); `--dev` = raw `main` branch (no checksum, print a warning).
4. Download to `mktemp -d`, then `exec bash "$tmp/zbx-install.sh" "$@"` so all original flags pass through — handing off to a Homebrew bash when the current one is < 4 (each candidate is version-probed; on Darwin with no modern bash anywhere, exit 3 with `brew install bash` instructions).

The bootstrap itself must be trivial enough to audit at a glance: no logic beyond the above.

**Windows** gets its own single-file bootstrap-plus-installer, `install.ps1` (published as a release asset like `install.sh`): `irm .../releases/latest/download/install.ps1 | iex`, or `& ([scriptblock]::Create((irm ...))) -Server IP -Yes` to pass parameters (`iex` cannot forward them). It is self-contained — agent-only scope doesn't justify a bundle — and integrity rests on the MSI's Authenticode signature check rather than a checksum file.
...
10. arm64: Zabbix repos cover most but not all family/version combos → if `DETECT_ARCH=aarch64` and the repo probe (§12.1) fails, say so explicitly in the error menu ("no repo for this arch/OS combo": agent-only fallback if the agent repo resolves · exit 3). Confirmed first-class: Amazon Linux 2023 (`aarch64`); the probe→agent-only fallback still applies to every unverified arm combo.
...
**Amazon Linux 2023** (`amzn`, family rhel) is a supported full-server target on `x86_64` and `aarch64` for both 7.0 and 7.4 (packages verified 2026-07). `detect_family` already maps `amzn`→rhel and package selection is family-based, so the only code specific to it is in `detect.sh` (support-matrix + `aarch64` promotion via `_arch_confirmed_for_os`, §15.10) and `repo.sh`'s DNF URL builder — Amazon Linux's release RPM lives at `.../amazonlinux/<VERSION_ID>/…` with an `.amzn<VERSION_ID>` suffix rather than the rhel `.../rhel/<major>/…el<major>` shape (the existing flat-vs-legacy HEAD probe still selects 7.0-flat vs 7.4-release). CI covers it via the `amazonlinux:2023` smoke image (x86_64); a systemd real-install job is deferred (no maintained systemd-in-Docker AL2023 image today) — the URL shapes are unit-tested and the repo was verified live instead.

**macOS** (`macos`, detected via `uname`) is a supported **agent-only** target on Apple Silicon (`arm64`) and Intel (`x86_64`) — Zabbix ships no macOS server/proxy, only the classic `zabbix_agentd`: a signed `.pkg` for arm64 and a tar.gz archive for amd64 on cdn.zabbix.com. A self-contained path in `os_macos.sh` (routed from `main()` for `DETECT_OS_ID=macos`) dispatches on arch (`_macos_variant`): the pkg path installs via `installer(8)` after a `pkgutil --check-signature` gate (temp file must end in `.pkg`); the tar path unpacks into the exact layout the .pkg produces (`/usr/local/{sbin,bin}`, conf seeded as `.conf.NEW`), codesign-verifies the binary (WARN when unsigned — TLS-only integrity), and hand-writes a launchd plist with the same label, so config/service/health/uninstall are shared. Both then point `zabbix_agentd.conf` at the server, load the launchd daemon, and health-check port 10050; `--uninstall` reverses either variant. macOS ships bash 3.2, so `core.sh` guards for bash ≥ 4 and `install.sh` hands off to a Homebrew bash. The pkg path is verified end-to-end on a real Apple-Silicon `macos-latest` CI runner; GitHub retired Intel runners, so the tar path is covered by a dry-run routing assertion plus a live CDN HEAD probe of the tar.gz URL in the same job.

**Windows** is a supported **agent-only** target via the native `install.ps1` (§4, §6) — a self-contained PowerShell analog of `os_macos.sh`, not part of the bash bundle. It shares the CDN `latest`-pointer idea (`.../stable/<major>/latest/zabbix_agent[2]-<major>-latest-windows-<arch>-openssl.msi`), gates on a valid Zabbix Authenticode signature, installs with msiexec public properties (SERVER/SERVERACTIVE/HOSTNAME), health-checks the service + port 10050 (exit 6 on failure), and uninstalls by product code from the registry. Verified end-to-end (Pester units + real install/uninstall of both agentd and agent2) on a `windows-latest` CI runner.

**Raspberry Pi OS / Debian-on-ARM is on hold (upstream packaging bug).** The detection + repo-routing were built and are correct, but a real arm64 install — validated natively in a Debian 12 arm64 container — fails: Zabbix's `raspbian` bookworm arm64 packages declare a `libssl1.1` dependency that is unsatisfiable on Debian 12 / Raspberry Pi OS 12 (which ship `libssl3`), so `apt` refuses them. The support was removed rather than ship a broken install; revisit when Zabbix rebuilds those packages against `libssl3`.

**Remaining roadmap:** flip to **Zabbix 8.0 LTS** when it reaches GA (see §4 — append to the version arrays; a scheduled re-check watches for GA).
