...
| Raspberry Pi OS | `raspbian` (32-bit) · `debian` (64-bit) | 12 (bookworm), **Zabbix 7.0 only** | apt | Debian family; full server on `armhf` + `arm64` via Zabbix's raspbian repo. 64-bit Pi OS reports `ID=debian` — routed by arch. 7.4 has no ARM packages (gated off). |
...
**Raspberry Pi OS 12** (bookworm, family debian) is a supported full-server target on `armhf` and `arm64` for **Zabbix 7.0 only**. The wrinkle is detection: 32-bit Pi OS reports `ID=raspbian`, but **64-bit Pi OS reports `ID=debian`** (indistinguishable from plain Debian by os-release), and Zabbix ships all Debian-family ARM packages only in the `raspbian` repo (the `debian` repo is x86-only). So the design keys off **arch, not hardware**: `detect_supported` accepts `raspbian`/12 (64-bit is already covered by the `debian`/12 row); `_arch_class` treats all ARM as repo-dependent `maybe`, promoted to `yes` for the verified combos via `_arch_confirmed_for_os` (amzn 2023, raspbian 12, debian 12); and `repo.sh` routes any Debian-family ARM host to `/raspbian/` with the `+debian<ver>` suffix (§15.10). 7.4 has no ARM packages in that tree, so a Debian-family ARM plan is hard-gated to 7.0 in `prepare_plan`. Verified by unit tests (detect + URL routing) and live repo probes; **no real ARM install has been run** — this project's CI is x86-only and there is no arm runner/QEMU path, so unlike Amazon Linux (which reuses the real-install-tested rhel x86 path), the Debian-family install pipeline has never executed on ARM. This is the one shipped platform without an end-to-end install behind it.
